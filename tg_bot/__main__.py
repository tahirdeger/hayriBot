...
for module_name in ALL_MODULES:
    ...
    if not imported_module.__mod_name__.lower() in IMPORTED:
        ...
        raise Exception("Aynı isme sahip modül var. Lütfen birinin adını değiştir.")
...
def help_button(bot: Bot, update: Update):
    ...
    try:
        if mod_match:
            ...
            text = "İşte *{}* modülü ile ilgili bilgiler:\n".format(HELPABLE[module].__mod_name__) \
...
def get_help(bot: Bot, update: Update):
    ...
    if chat.type != chat.PRIVATE:
        ...
        update.effective_message.reply_text("Olası komutların listesi için seninle özelde görüşelim.",
                                            reply_markup=InlineKeyboardMarkup(
...
def send_settings(chat_id, user_id, user=False):
    if user:
        if USER_SETTINGS:
            ...
            dispatcher.bot.send_message(user_id, "Mevcut ayarlar bunlar:" + "\n\n" + settings,
                                        parse_mode=ParseMode.MARKDOWN)

        else:
            dispatcher.bot.send_message(user_id, "Kullanıcıya özel bir ayar yok gibi görünüyor:'(",
                                        parse_mode=ParseMode.MARKDOWN)
        ...
        if CHAT_SETTINGS:
            ...
            dispatcher.bot.send_message(user_id,
                                        text="Hangi modül için {} ayarlarını kontrol etmek istersin??".format(
                                            chat_name),
                                        reply_markup=InlineKeyboardMarkup(
...
def settings_button(bot: Bot, update: Update):
    ...
    try:
        if mod_match:
            ...
            chat = bot.get_chat(chat_id)
            query.message.reply_text("Vay arkadaş! {} için epeyce ayar var - Devam et ve "
                                     "ilgilendiğin şeyi seç.".format(chat.title),
                                     reply_markup=InlineKeyboardMarkup(
            ...
            chat = bot.get_chat(chat_id)
            query.message.reply_text(text="Vay arkadaş! {} için epeyce ayar var - Devam et ve "
                                          "ilgilendiğin şeyi seç.".format(escape_markdown(chat.title)),
                                     parse_mode=ParseMode.MARKDOWN,
...
def get_settings(bot: Bot, update: Update):
    ...
    if chat.type != chat.PRIVATE:
        if is_user_admin(chat, user.id):
            text = "Yazışma ayarlarınızı yapmak için buraya tıklayın"
            msg.reply_text(text,
                           reply_markup=InlineKeyboardMarkup(
            ...
            text = "Ayarlarını gözden geçirmek için tıkla."
...
def donate(bot: Bot, update: Update):
    ...
    if chat.type == "private":
        ...
        try:
            ...
            update.effective_message.reply_text("İşverenime destek için sana mesaj gönderdim!")
        except Unauthorized:
            update.effective_message.reply_text("Destek için özelde görüşelim.")
...
def migrate_chats(bot: Bot, update: Update):
    ...
    LOGGER.info("%s 'den, %s 'e taşınıyor", str(old_chat), str(new_chat))
    for mod in MIGRATEABLE:
        mod.__migrate__(old_chat, new_chat)

    LOGGER.info("Taşıma başarılı!")
...
def main():
    ...
    donate_handler = CommandHandler("bagis", donate)
...
def process_update(self, update):
    try:
        ...
        self.logger.exception('id ERROR')
