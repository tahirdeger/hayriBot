    ...
    if user_id:
        if update.effective_message.reply_to_message and update.effective_message.reply_to_message.forward_from:
            ...
            update.effective_message.reply_text(
                "Orijinal gönderen, {}, şu ID kimliğine sahip `{}`.\nİletici, {}, şu ID kimliğine sahip `{}`.".format(
            ...
            update.effective_message.reply_text("{}'nin ID'si `{}`.".format(escape_markdown(user.first_name), user.id),
                                                parse_mode=ParseMode.MARKDOWN)
    else:
        chat = update.effective_chat  # type: Optional[Chat]
        if chat.type == "private":
            update.effective_message.reply_text("Senin ID `{}`.".format(chat.id),
                                                parse_mode=ParseMode.MARKDOWN)
...
def info(bot: Bot, update: Update, args: List[str]):
    ...
    if user_id:
        ...
        msg.reply_text("Bundan bir kullanıcıyı çıkaramıyorum.")
        return

    else:
        return

    text = "<b>Kişi bilgileri</b>:" \
           "\nID: <code>{}</code>" \
           "\nAdı: {}".format(user.id, html.escape(user.first_name))

    if user.last_name:
        text += "\nSoyadı: {}".format(html.escape(user.last_name))

    if user.username:
        text += "\nKullanıcıadı: @{}".format(html.escape(user.username))

    text += "\nKullanıcı linki: {}".format(mention_html(user.id, "link"))
    ...
    if user.id == OWNER_ID:
        ...
        if user.id in SUDO_USERS:
            text += "\nBu kişi sudo kullanıcılarımdan biri! " \
                    "Neredeyse sahibim kadar güçlü - öyleyse izle."
        else:
            if user.id in SUPPORT_USERS:
                text += "\nBu kişi destek kullanıcılarımdan biri! " \
                        "Tam olarak bir sudo kullanıcısı değil, ama yine de sizi haritadan uzaklaştırabilir."

            if user.id in WHITELIST_USERS:
                text += "\nBu kişi beyaz listeye alındı! " \
                        "Bu onları yasaklamama / tekmeleme iznim olmadığı anlamına geliyor."
