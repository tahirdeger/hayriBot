...
def havanasil(bot: Bot, update: Update):
    if update.effective_message:
        try:
            ...
            update.effective_message.reply_text(secim + " şehrindeki hava bugün şöyle:")
            update.effective_message.reply_text(v1)
            update.effective_message.reply_text(v2)
            update.effective_message.reply_text(v3)
        except Exception:
            msg.reply_text("Şehir seçimin yanlış. Böyle bir şehir bulamadım. Harf hatan olabilir belki !")
        
    else:
        msg.reply_text("Şehir adını doğru girmen gerekiyor, değil mi??")
