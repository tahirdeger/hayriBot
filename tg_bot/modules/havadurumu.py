...
class HavaDurumu:
    def __init__(self):
        ...
        self.api_key = "1408d15a21c820caa11a952c18b87675"


    def sehirID(self , sehir):
    ...
    def anlikBilgi(self, id):
        site = self.api_url + "id=" + id + "&APPID=" + self.api_key
    ...
    def veriAl(self, sicaklik , hissedilen , nem , gokyuzu):
        ...
        return(f"""{secim} şehrinde hava bilgileri:
                {v1}
                {v2}
                {v3}
                """)
...
def havanasil(bot: Bot, update: Update):
    if update.effective_message:
        ...
        secim = msg.text
        try:
            sehir = capitalize(secim)
            veri = havaDurumu.sehirID(sehir)
            veriListesi = havaDurumu.anlikBilgi(veri)
            hava = havaDurumu.veriAl(veriListesi[0],veriListesi[1],veriListesi[2],veriListesi[3])
            update.effective_message.reply_text(hava)
        except Exception:
            print("Hata")
