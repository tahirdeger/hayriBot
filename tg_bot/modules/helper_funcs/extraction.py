...
def extract_user_and_text(message: Message, args: List[str]) -> (Optional[int], Optional[str]):
    ...
    if entities and ent and ent.offset == len(message.text) - len(text_to_parse):
        ...
        if not user_id:
            message.reply_text("Veritabanımda o kullanıcı yok. Bunun yerine o kişinin mesajını yanıtlarsanız "
                               "veya o kullanıcının mesajlarından birini iletirseniz onlarla etkileşime girebilirsiniz.")
            return None, None
    ...
    try:
        ...
        if excp.message in ("User_id_invalid", "Chat not found"):
            message.reply_text("Görünüşe göre bu kullanıcıyla daha önce etkileşime girmemiştim"
                               "kontrolü bana vermek için lütfen onlardan bir mesaj iletin!"
                               "Belirli komutları uygulayabilmek için onlardan bir parçaya ihtiyacım var)")
        else:
